...
async def test_websocket_chat_streaming(db_session):
    try:
        ...
        conv = Conversation(persona_id=persona.id, title="Debate Session")
        db_session.add(conv)
        await db_session.commit()
        ...
        async def mock_generate_chat_stream(
            self, system_instruction, chat_history, user_message, temperature=0.8
        ):
        ...
        with patch("google.genai.models.AsyncModels.embed_content", mock_embed_content):
            with patch.object(
                ...
                await mock_ws.receive_queue.put(
                    {"type": "user_message", "text": "Why is sky blue?"}
                )
