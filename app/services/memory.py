...
class MemoryService:
    ...
    async def ingest_document(
        ...
        logger.info(
            f"Successfully ingested document: {filename} with {len(chunks)} chunks."
        )
        ...
        async with async_session_maker() as db:
            ...
            try:
                ...
                stmt = (
                    ...
                    .where(
                        distance < 0.7
                    )  # Cosine similarity threshold (distance < 0.7 means similarity > 0.3)
    ...
    async def get_preamble_memories(
        self, persona_id: uuid.UUID, conversation_id: uuid.UUID | None
    ) -> list[Memory]:
        """
        Retrieves preamble memories (latest summary + top facts by recency/importance)
        for the start of a Live session without making embedding calls.
        """
        memories = []
        async with async_session_maker() as db:
            if conversation_id:
            ...
            memories.extend(facts_res.scalars().all())
